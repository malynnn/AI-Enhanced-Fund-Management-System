...
function TreasurerDashboardContent() {
  ...
  // Preview States for Transaction Ledger
  const [selectedFund, setSelectedFund] = useState<any>(null);
  const [ledger, setLedger] = useState<any[]>([]);
  const [searchTerm, setSearchTerm] = useState('');

  // Hydration Safe Init & Fetch Data
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      try {
        ...
        // Fetch Basic Fund Balances
        const dashRes = await fetch(`${gatewayUrl}/api/finance/dashboard`);
        if (dashRes.ok) {
          const dashData = await dashRes.json();
          setFunds(dashData.funds || []);
          setLedger(dashData.ledger || []);
          if (dashData.funds && dashData.funds.length > 0) setSelectedFund(dashData.funds[0]);
        }

        // Fetch AI Analytics Data
        const analyticsRes = await fetch('/api/forecasting/analytics', { 
          method: 'POST', 
  ...
  // Robust Markdown Parser for AI Recommendations (TS Error Fixed)

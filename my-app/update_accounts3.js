...
content = content.replace(/const res = await fetch\(`\$\{gatewayUrl\}\/api\/finance\/accounts`\);/g, "const res = await fetch(`${gatewayUrl}/api/finance/accounts`, { headers: { 'Authorization': `Bearer ${token}` } });");

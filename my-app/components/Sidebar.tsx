...
export default function Sidebar() {
  ...
  // FIX 1: If on the login page, render absolutely nothing (kills the gray sidebar space)
  ...
  return (
    // FIX 2: The <aside> wrapper is now here. It only renders if we are NOT on /login.
      ...
      {status === "loading" ? (
        ...
        <div className={`flex flex-col h-full text-white px-6 py-8 transition-colors duration-300 ${sidebarBgColor}`}>
          ...
          <div className="mt-auto pt-6">
            ...
            {/* FIX 3: callbackUrl changed from '/' to '/login' */}
            <button 
              onClick={() => signOut({ callbackUrl: '/login' })} 
              className="w-full py-2.5 bg-bdoea-yellow hover:bg-yellow-500 text-black font-semibold text-sm rounded-md transition-colors flex items-center justify-center gap-2"
            >
